...
1. HTML ファイルを開き、次の情報を入力します。
    ...
    - **管理者のユーザー** 名-サーバー管理者のユーザー名。 ログイン名には次の要件が満たされていることを確認してください。ログイン名には、SQL 識別子または一般的なシステム名 (admin、administrator、sa、root、dbmanager、loginmanager など) または組み込みのデータベースユーザーまたはロール (dbo、guest、public など) を含めることはできません。ログイン名に英数字以外の文字 (空白文字、Unicode 文字を含む) を含めることはできません。ログイン名の先頭に数字や記号を使用することはできません

    - **管理者パスワード** -サーバー管理者パスワード。 パスワードの長さは8文字以上である必要があります。
パスワードの長さは128文字以下でなければなりません。
パスワードには、英大文字、英小文字、数字 (0 ～ 9)、英数字以外の文字 (!、$、#、% など) のうち、3 つのカテゴリの文字が含まれている必要があります。
パスワードには、ログイン名の全部または一部を含めることはできません。 (ログイン名の一部は、3文字以上の連続する英数字として定義されています)。
...
1. HTML ファイルを開き、次の情報を入力します。
    ...
    - **インスタンス管理者のユーザー名** –インスタンス管理者のユーザー名。 ログイン名には次の要件が満たされていることを確認してください。ログイン名には、SQL 識別子または一般的なシステム名 (admin、administrator、sa、root、dbmanager、loginmanager など) または組み込みのデータベースユーザーまたはロール (dbo、guest、public など) を含めることはできません。ログイン名に英数字以外の文字 (空白文字、Unicode 文字を含む) を含めることはできません。ログイン名の先頭に数字や記号を使用することはできません

    - **インスタンス管理者パスワード** -インスタンス管理者パスワード。 パスワードの長さは16文字以上である必要があります。
パスワードの長さは128文字以下でなければなりません。
パスワードには、英大文字、英小文字、数字 (0 ～ 9)、英数字以外の文字 (!、$、#、% など) のうち、3 つのカテゴリの文字が含まれている必要があります。
パスワードには、ログイン名の全部または一部を含めることはできません。 (ログイン名の一部は、3文字以上の連続する英数字として定義されています)。
