...
[!INCLUDE[sssql15-md](../includes/sssql16-md.md)] 以降では、割り当てが [IAM チェーン](#IAM)の最初の 8 ページに属している場合を除き、ユーザー データベースと tempdb のほとんどの割り当てで、単一エクステントが既定値として使用されます。 master、msdb、model の各データベースの割り当てでは、以前の動作がそのまま保持されます。 
