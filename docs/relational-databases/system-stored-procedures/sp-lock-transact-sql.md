...
`[ @spid2 = ] 'session ID2'`[!INCLUDE[ssDE](../../includes/ssde-md.md)] **Sys.dm_exec_sessions** からの別のセッション ID 番号です。これは、*セッション ID1* と同時にロックを持つ可能性があり、ユーザーも情報を必要とします。 *SESSION ID2* は **int** で、既定値は NULL です。  
...
|列名|データ型|説明|  
|-----------------|---------------|-----------------|  
|**spid**|**smallint**|[!INCLUDE[ssDE](../../includes/ssde-md.md)]ロックを要求しているプロセスのセッション ID 番号。|  
