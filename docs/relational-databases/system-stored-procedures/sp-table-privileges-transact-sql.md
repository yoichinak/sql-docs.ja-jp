...
|GRANTEE|**sysname**|この TABLE_NAME の権限を、表示される GRANTOR によって与えられたデータベース ユーザーの名前。 では [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 、この列には常に sys.database_principalssystem ビューのデータベースユーザーが含まれています。 このフィールドは常に値を返します。|  
