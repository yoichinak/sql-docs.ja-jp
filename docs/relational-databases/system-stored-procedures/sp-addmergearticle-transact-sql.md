...
`[ @status = ] 'status'` アーティクルの状態を示します。 *状態* は **nvarchar (10)**,、既定値は **同期** されていません。 **アクティブ** になっている場合は、テーブルをパブリッシュする初期処理スクリプトが実行されます。 **同期** されていない場合、テーブルをパブリッシュする初期処理スクリプトは、次にスナップショットエージェントを実行したときに実行されます。  
