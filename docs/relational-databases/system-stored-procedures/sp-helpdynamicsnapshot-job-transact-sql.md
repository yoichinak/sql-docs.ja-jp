...
`[ @dynamic_snapshot_jobid = ] 'dynamic_snapshot_jobid'` フィルター選択されたデータスナップショットジョブの識別子を選択します。 *dynamic_snapshot_jobid* は **uniqueidentifier**,、既定値は NULL の場合、指定した *dynamic_snapshot_jobname* に一致するすべてのスナップショットジョブを返します。  
