...
## <a name="encryption-at-rest-utilities-on-big-data-cluster-bdc"></a>ビッグ データ クラスター (BDC) での保存時の暗号化ユーティリティ

このセクションには、BDC で保存時の暗号化機能を管理するのに役立つ一連のノートブックが含まれています。

|名前 |説明 |
|---|---|
|SOP0124 - 保存時の暗号化のキーを一覧表示する|このノートブックを使用し、すべての HDFS キーを一覧表示します。|
|SOP0128 - ビッグ データ クラスターで HDFS 暗号化ゾーンを有効にする|このノートブックを使用し、CU6 以前から CU8 にアップグレードするときに HDFS 暗号化ゾーンを有効にします。 CU8 以降の新規展開時、または CU9 にアップグレードするときには必要ありません。|
|SOP0125 - 保存時の暗号化のキーを削除する|このノートブックを使用し、HDFS 暗号化ゾーン キーを削除します。 __注意__|
|SOP0126 - 保存時の暗号化用のキーをバックアップする|このノートブックを使用し、HDFS 暗号化ゾーン キーをバックアップします。|
|SOP0127 - 保存時の暗号化用のキーを復元する|このノートブックを使用し、HDFS 暗号化ゾーン キーを復元します。|
