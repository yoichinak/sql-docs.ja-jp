...
|プロパティ名|データ型|説明|  
|-------------------|---------------|-----------------|  
|Connection|ADO.Net Connection|変更テーブルにアクセスするための [!INCLUDE[ssnoversion](../../includes/ssnoversion-md.md)] CDC データベースへの ADO.NET 接続。|  
...
|CommandTimeOut|整数|この値は、 [!INCLUDE[ssnoversion](../../includes/ssnoversion-md.md)] データベースと通信する際に使用されるタイムアウト (秒単位) を示します。 この値は、データベースからの応答時間が非常に遅い場合に使用されるため、既定値 (30 秒) は不十分です。|  
