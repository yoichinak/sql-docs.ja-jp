...
7. **[テーブルの作成]** ダイアログ ボックスで、次の操作を行います。
   ...
   1. 変換先テーブルの名前を **SalesOrderDetail** に変更します。
       ...
       ![[テーブルの作成] ダイアログ ボックスのスクリーンショット。 SalesOrderDetail という名前のテーブルを作成する SQL コードが表示されています。][12b]
...
![ソース アダプターと変換先アダプターを示すスクリーンショット。 各アダプターの上に黄色の糸車があり、それらの間に "89748 行" というテキストがあります。][14]
