---
title: Configuration 要素 (DTA)
description: dta ユーティリティでは、Configuration 要素により、既存の仮想物理設計構造から成るユーザー指定の構成を指定します。
ms.prod: sql
