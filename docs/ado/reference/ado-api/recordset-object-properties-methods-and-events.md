---
description: Recordset オブジェクトのプロパティ、メソッド、およびイベント
title: Recordset オブジェクトのプロパティ、メソッド、およびイベント |Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: ado
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: reference
helpviewer_keywords:
- Recordset object [ADO], members
ms.assetid: 4295a6e5-112d-4595-b18a-57728893ac2d
author: rothja
ms.author: jroth
ms.openlocfilehash: 0446f74eafda87235223e5906fd7e665a201ab0a
ms.sourcegitcommit: 917df4ffd22e4a229af7dc481dcce3ebba0aa4d7
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 02/10/2021
ms.locfileid: "100051773"
---
# <a name="recordset-object-properties-methods-and-events"></a>Recordset オブジェクトのプロパティ、メソッド、およびイベント
## <a name="propertiescollections"></a>プロパティ/コレクション  
 [AbsolutePage プロパティ](./absolutepage-property-ado.md)  
  
 [AbsolutePosition プロパティ](./absoluteposition-property-ado.md)  
  
 [ActiveCommand のプロパティ](./activecommand-property-ado.md)  
  
 [ActiveConnection プロパティ](./activeconnection-property-ado.md)  
  
 [BOF、EOF プロパティ](./bof-eof-properties-ado.md)  
  
 [Bookmark プロパティ](./bookmark-property-ado.md)  
  
 [CacheSize プロパティ](./cachesize-property-ado.md)  
  
 [カーソル位置プロパティ](./cursorlocation-property-ado.md)  
  
 [CursorType プロパティ](./cursortype-property-ado.md)  
  
 [DataMember プロパティ](./datamember-property.md)  
  
 [DataSource プロパティ](./datasource-property-ado.md)  
  
 [EditMode プロパティ](./editmode-property.md)  
  
 [Fields コレクション](./fields-collection-ado.md)  
  
 [Filter プロパティ](./filter-property.md)  
  
 [Index プロパティ](./index-property.md)  
  
 [LockType プロパティ](./locktype-property-ado.md)  
  
 [MarshalOptions プロパティ](./marshaloptions-property-ado.md)  
  
 [MaxRecords プロパティ](./maxrecords-property-ado.md)  
  
 [PageCount プロパティ](./pagecount-property-ado.md)  
  
 [PageSize プロパティ](./pagesize-property-ado.md)  
  
 [プロパティのコレクション](./properties-collection-ado.md)  
  
 [RecordCount プロパティ](./recordcount-property-ado.md)  
  
 [Sort プロパティ](./sort-property.md)  
  
 [Source プロパティ (ADO Recordset)](./source-property-ado-recordset.md)  
  
 [State プロパティ](./state-property-ado.md)  
  
 [Status プロパティ (ADO Recordset)](./status-property-ado-recordset.md)  
  
 [StayInSync プロパティ](./stayinsync-property.md)  
  
## <a name="methods"></a>メソッド  
 [AddNew メソッド](./addnew-method-ado.md)  
  
 [Cancel メソッド](./cancel-method-ado.md)  
  
 [CancelBatch メソッド](./cancelbatch-method-ado.md)  
  
 [CancelUpdate メソッド](./cancelupdate-method-ado.md)  
  
 [Clone メソッド](./clone-method-ado.md)  
  
 [Close メソッド](./close-method-ado.md)  
  
 [CompareBookmarks メソッド](./comparebookmarks-method-ado.md)  
  
 [Delete メソッド (ADO Recordset)](./delete-method-ado-recordset.md)  
  
 [Find メソッド](./find-method-ado.md)  
  
 [GetRows メソッド](./getrows-method-ado.md)  
  
 [GetString メソッド](./getstring-method-ado.md)  
  
 [Move メソッド](./move-method-ado.md)  
  
 [MoveFirst、MoveLast、MoveNext、および MovePrevious メソッド](./movefirst-movelast-movenext-and-moveprevious-methods-ado.md)  
  
 [NextRecordset メソッド](./nextrecordset-method-ado.md)  
  
 [Open メソッド (ADO Recordset)](./open-method-ado-recordset.md)  
  
 [Requery メソッド](./requery-method.md)  
  
 [Resync メソッド](./resync-method.md)  
  
 [Save メソッド](./save-method.md)  
  
 [Seek メソッド](./seek-method.md)  
  
 [Supports メソッド](./supports-method.md)  
  
 [Update メソッド](./update-method.md)  
  
 [UpdateBatch メソッド](./updatebatch-method.md)  
  
## <a name="events"></a>イベント  
 [EndOfRecordset イベント](./endofrecordset-event-ado.md)  
  
 [FetchComplete イベント](./fetchcomplete-event-ado.md)  
  
 [FetchProgress イベント](./fetchprogress-event-ado.md)  
  
 [Changefield および FieldChangeComplete イベント](./willchangefield-and-fieldchangecomplete-events-ado.md)  
  
 [Changerecord イベントと RecordChangeComplete イベントを発生させる](./willchangerecord-and-recordchangecomplete-events-ado.md)  
  
 [WillChangeRecordset イベントと RecordsetChangeComplete イベント](./willchangerecordset-and-recordsetchangecomplete-events-ado.md)  
  
 [移動イベントと MoveComplete イベントを発生させる](./willmove-and-movecomplete-events-ado.md)  
  
## <a name="see-also"></a>参照  
 [Recordset オブジェクト (ADO)](./recordset-object-ado.md)