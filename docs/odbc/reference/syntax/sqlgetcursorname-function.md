...
|01004|文字列データ、右側が切り捨てられました|バッファーカーソル名がカーソル名全体を返すの \* に十分な大きさではないため、カーソル名が切り詰められました。 切り捨てられていないカーソル名の長さは **NameLengthPtr* で返されます。 (関数は SQL_SUCCESS_WITH_INFO を返します)。|  
