...
|**SQLBulkOperations**|ブックマーク操作による挿入操作、または更新、削除、またはフェッチを実行します。 実装の詳細は次のとおりです。<br /><br /> -アプリケーションが ODBC 2.x ドライバーで SQL_ADD 操作を使用して **Sqlbulkoperations** を呼び出すと、odbc *3.x ドライバーマネージャー* は、SQL_ADD *操作* を使用して、その操作を **SQLSetPos** にマップ *します。* <br />-SQL_ADD *操作* による **sqlsetpos** をサポートしていない odbc 2.x ドライバーを使用している場合 *、odbc* 3.x ドライバーマネージャーでは、SQL_ADD 操作によって **Sqlsetpos** が SQL_ADD *操作* によって **sqlbulkoperations** にマップされることはありません。  これは、 **Sqlbulkoperations** を state S7 で呼び出すことができないためです *。 ODBC 2.x では、* **SQLSetPos** を呼び出すことができませんでした。<br />- **Sqlbulkoperations** を呼び出す前に、アプリケーションが odbc 2.x ドライバーで SQL_ADD *操作* を使用して **sqlbulkoperations** を呼び出した場合 *、odbc* *3.x ドライバーマネージャー* はエラーを返します。|  
